...
for(int i=1;i<=xl.rowCount(TestCases);i++)
{
	...
	{
		//store all sheets into TC Module
		...
		for(int j=1;j<=xl.rowCount(TCModule);j++)
		{
			...
			try {
				...
				{
					FunctionLibrary.captureSupplierNum(driver, Locator_Type, Locator_Value);
					logger.log(LogStatus.INFO,Description);
				}
				...
				{
					FunctionLibrary.captureCustomerNumber(driver, Locator_Type, Locator_Value);
				...
				//write as status PASS in TCModule
		...
		//write as blocked into status cell of Testcases sheet
